...
export default function InvoicePaggination({ invoice, limit, setPageNo, pageNo }) {
    console.log("page : ", invoice);
    ...
    return (
        <div className="pagination-festum">
            <p className="show-text">Showing {(invoice?.page - 1) * limit + 1} to {Math.min((invoice?.page) * limit, invoice?.totalDocs)} Of {invoice?.totalDocs} Invoice</p>
            <ul className="">
                <li>
